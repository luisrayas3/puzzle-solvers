...
static constexpr size_t SQUARE_SIDE = 3;
static constexpr size_t BOARD_SIZE = SQUARE_SIDE * SQUARE_SIDE;

using Cell = uint16_t;
static_assert(sizeof(Cell) * 8 >= BOARD_SIZE, "Cell width not big enough!");

static constexpr Cell UNKNOWN = (1 << BOARD_SIZE) - 1;
...
int main(int argc, char* argv[])
{
  ...
  using SetGetter = void (*)(Cell /*board*/[][BOARD_SIZE], size_t /*idx*/, Cell* /*res*/[]);
  SetGetter set_getters[] = {column, row, block, nullptr};

  int num_iters = 0;
  while (!stop(board)) {
    Cell* cells[BOARD_SIZE];
    for (SetGetter* getSet = &set_getters[0]; *getSet != nullptr; ++getSet) {
...
uint16_t transposeBits(const Cell* const cells[], size_t bit_i);
